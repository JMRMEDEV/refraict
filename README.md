...
```json
{
  ...
  "analysis": {
    ...
    "detect_text_tiers": true,
    "detect_text_weight": true,
    "no_ocr": false,
    "no_summary": false
  },
  "cache": {
...
| Tool | Purpose |
| --- | --- |
| `analyze` | Run the full pipeline on an image. Returns a **lean** bounded summary — page type + confidence, component counts by type, and roll-up **counts** for the derived signals (`corner_styles` rounded/square/measured, `padding_containers`, `text_tiers` bands, `text_weights` heavy/regular + a few sample bold words, `layout_containers` inferred columns/rows, `evenly_spaced_groups`) plus the `grounding`/`crosscheck`/`consolidation_check` scores and `output_dir`. Per-item detail comes from the query tools below, not from re-reading a whole artifact. |
